...
def askquestionsport():  # categorie sport #(thenewboston, 2014)
    ...
    while not timed == 0:
        ...
        messages_to_screen(l[random_selection].answer3[0], black, 20, 3.3)
        messages_to_screen("Kies A , B of C ", black, 20, 2.6)
        messages_to_screen(str(timed / 375), black, 20, 2.6)
        pygame.display.update()
        timed -= 1
        # knopjes linken
        for event in pygame.event.get():
...
def main_menu():
    ...
    while intro:
        ...
        if state == 0:
            ...
            menuDisplay.blit(pygame.transform.scale(background, (display_width, display_height)), (0, 0))
            Button(score_List, (display_width * 0.02), (display_height * 0.02),
                   (display_width - (display_width * 0.04)), ((display_height * 0.7) - (display_width * 0.04)),
                   sky_blue, lightsky_blue)
